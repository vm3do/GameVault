...
require __DIR__ . '/usersManagement.php';
require_once __DIR__ . '/gameCRUD.php';
...
<body class="bg-gray-900 text-white">
          ...
          <tbody class="divide-y divide-gray-600">
            <?php foreach ($games as $game): ?>
              <tr>
                ...
                <td class="px-6 py-4">
                  <a href="dashboard.php?action=editGame&Id=<?php echo $game['id']; ?>"
                    class="bg-violet-accent px-4 py-2 rounded-lg hover:bg-violet-700 transition">Edit</a>
                  <a href="dashboard.php?action=deleteGame&Id=<?php echo $game['id']; ?>"
                    class="bg-red-500 px-4 py-2 rounded-lg hover:bg-red-600 transition ml-2">Delete</a>
                </td>
              </tr>
            <?php endforeach; ?>
          </tbody>
  ...
  <!-- Manage Users Modal -->
  <div id="manageUsersModal" class="fixed inset-0 bg-gray-900 bg-opacity-75 flex items-center justify-center hidden">
                ...
                <td class="px-6 py-4">
                  <div class="flex space-x-2">
                    <form action="" method="POST">
                      ...
                      <button type="submit"
                        class="bg-violet-accent px-4 py-2 rounded-lg hover:bg-violet-700 transition">Upgrade</button>
                    </form>
                    <?php if ($user['status'] == 'active'): ?>
                      <form action="" method="POST">
                        <input type="hidden" name="ban" value="<?php echo $user['id']; ?>">
                        <button type="submit"
                          class="bg-red-500 px-4 py-2 rounded-lg hover:bg-red-600 transition ml-2">Ban</button>
                      </form>
                    <?php endif; ?>
                    <?php if ($user['status'] == 'banned'): ?>
                      <form action="" method="POST">
                        ...
                        <button type="submit"
                          class="bg-red-500 px-4 py-2 rounded-lg hover:bg-red-600 transition ml-2">Unban</button>
                      </form>
                    <?php endif; ?>
                  </div>
                </td>
  ...
  <!-- Add Game Modal -->
  <div id="addGameModal" class="fixed inset-0 bg-gray-900 bg-opacity-75 flex items-center justify-center hidden">
    <div class="bg-gray-800 p-6 rounded-lg w-full max-w-md mx-4">
      <!-- Modal Header -->
      <div class="flex justify-between items-center mb-4">
        <h2 class="text-xl font-bold">Add Game</h2>
        <button onclick="closeAddGameModal()" class="text-gray-400 hover:text-white transition">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
          </svg>
        </button>
      ...
      <form action="dashboard.php" method="POST" id="addGameForm" class="space-y-4 modal-content">
        ...
        <div class="flex items-center gap-2">
          ...
          <button type="button" id="fetchButton"
        ...
        <div>
          ...
          <input type="url" id="scrshot1" name="scrshot1"
            class="mt-1 block w-full bg-gray-700 border border-gray-600 rounded-lg p-2 text-white custom-input focus:border-violet-accent focus:ring-violet-accent"
            required />
        </div>
        <!-- Screenshot 2 -->
        <div>
          ...
          <input type="url" id="scrshot2" name="scrshot2"
            class="mt-1 block w-full bg-gray-700 border border-gray-600 rounded-lg p-2 text-white custom-input focus:border-violet-accent focus:ring-violet-accent"
            required />
        </div>
        <!-- Screenshot 3 -->
        <div>
          ...
          <input type="url" id="scrshot3" name="scrshot3"
            class="mt-1 block w-full bg-gray-700 border border-gray-600 rounded-lg p-2 text-white custom-input focus:border-violet-accent focus:ring-violet-accent"
            required />
        </div>
        <!-- Rating -->
  ...
  <?php if (isset($_GET['Id']) && isset($_GET['action']) && $_GET['action'] === 'editGame'):
    ...
    ?>
    <div id="updateGameModal" class="fixed inset-0 bg-gray-900 bg-opacity-75 flex items-center justify-center">
      <div class="bg-gray-800 p-6 rounded-lg w-full max-w-md mx-4">
        <!-- Modal Header -->
        <div class="flex justify-between items-center mb-4">
          <h2 class="text-xl font-bold">Update Game</h2>
          <button onclick="closeUpdateGameModal()" class="text-gray-400 hover:text-white transition">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
            </svg>
          </button>
        ...
        <form action="dashboard.php" method="POST" class="space-y-4 modal-content">
          <?php if ($gameById): ?>
            ...
            <div class="flex items-center gap-2">
              <div class="flex-1">
                <label for="gameTitle" class="block text-sm font-medium text-gray-400">Title</label>
                <input type="text" id="gameTitle" name="new_title" value="<?php echo $gameById['title']; ?>"
                  class="mt-1 block w-full bg-gray-700 border border-gray-600 rounded-lg p-2 text-white custom-input focus:border-violet-accent focus:ring-violet-accent"
                  required />
              </div>
              <button type="button" id="fetchButton"
            ...
            <div>
              <label for="gameGenre" class="block text-sm font-medium text-gray-400">Genre</label>
              <input type="text" id="gameGenre" name="new_genre" value="<?php echo $gameById['genre']; ?>"
                class="mt-1 block w-full bg-gray-700 border border-gray-600 rounded-lg p-2 text-white custom-input focus:border-violet-accent focus:ring-violet-accent"
                required />
            </div>
            <!-- Description -->
            <div>
              ...
              <textarea name="new_description" rows="4"
                class="mt-1 block w-full bg-gray-700 border border-gray-600 rounded-lg p-2 text-white custom-input focus:border-violet-accent focus:ring-violet-accent"
                required><?php echo $gameById['description']; ?></textarea>
            </div>
            <!-- Release Date -->
            <div>
              <label for="releaseDate" class="block text-sm font-medium text-gray-400">Release Date</label>
              <input type="date" id="releaseDate" name="new_releaseDate" value="<?php echo $gameById['release_date']; ?>"
                class="mt-1 block w-full bg-gray-700 border border-gray-600 rounded-lg p-2 text-white custom-input focus:border-violet-accent focus:ring-violet-accent"
                required />
            </div>
            <!-- Image URL -->
            <div>
              <label for="background" class="block text-sm font-medium text-gray-400">Image URL</label>
              <input type="url" id="background" name="new_background" value="<?php echo $gameById['background']; ?>"
                class="mt-1 block w-full bg-gray-700 border border-gray-600 rounded-lg p-2 text-white custom-input focus:border-violet-accent focus:ring-violet-accent"
                required />
            </div>
            <!-- Screenshot 1 -->
            <div>
              <label for="scrshot1" class="block text-sm font-medium text-gray-400">Screenshot 1 URL</label>
              <input type="url" id="scrshot1" name="new_scrshot1" value="<?php echo $gameById['scrshot1']; ?>"
                class="mt-1 block w-full bg-gray-700 border border-gray-600 rounded-lg p-2 text-white custom-input focus:border-violet-accent focus:ring-violet-accent"
                required />
            </div>
            <!-- Screenshot 2 -->
            <div>
              <label for="scrshot2" class="block text-sm font-medium text-gray-400">Screenshot 2 URL</label>
              <input type="url" id="scrshot2" name="new_scrshot2" value="<?php echo $gameById['scrshot2']; ?>"
                class="mt-1 block w-full bg-gray-700 border border-gray-600 rounded-lg p-2 text-white custom-input focus:border-violet-accent focus:ring-violet-accent"
                required />
            </div>
            <!-- Screenshot 3 -->
            <div>
              <label for="scrshot3" class="block text-sm font-medium text-gray-400">Screenshot 3 URL</label>
              <input type="url" id="scrshot3" name="new_scrshot3" value="<?php echo $gameById['scrshot3']; ?>"
                class="mt-1 block w-full bg-gray-700 border border-gray-600 rounded-lg p-2 text-white custom-input focus:border-violet-accent focus:ring-violet-accent"
                required />
            </div>
            <!-- Rating -->
            <div>
              <label for="rating" class="block text-sm font-medium text-gray-400">Rating</label>
              <input type="number" id="rating" name="new_rating" value="<?php echo $gameById['rating']; ?>" min="0" max="10"
                step="0.1"
                class="mt-1 block w-full bg-gray-700 border border-gray-600 rounded-lg p-2 text-white custom-input focus:border-violet-accent focus:ring-violet-accent"
                required />
            </div>
            ...
            <div class="flex justify-end">
              ...
              <button type="submit" name="updateGame"
                class="bg-violet-accent px-4 py-2 rounded-lg hover:bg-violet-700 transition">
                Save Changes
              </button>
            </div>
